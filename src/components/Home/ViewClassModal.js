...
import ItemViewClass from './ItemViewClass'

const ViewClassModal = ({ modalVisible, setModalVisible, itemToRenderClass }) => {

    const paddingItem = (itemToRenderClass, incrementForRow) => {
        let pad = 0
        itemToRenderClass.map((item) => {
            console.log(item)
            let x = item.name.length / 30
        ...
        >
            <View style={styles.centeredView}>
                <View style={styles.modalView}>
                    <MyHeaderText fontSize={20} color={'white'} style={{ marginBottom: 10 }}>Clases Finalizadas</MyHeaderText>
                    {
                        (itemToRenderClass.length == 0)
                            ...
                            <LinearGradient
                                ...
                                <MyAppText fontSize={15} color={'white'} style={{ textAlign: 'center' }}>Aún no has finalizado ninguna clase</MyAppText>
                            </LinearGradient>
                            :
                            <FlatList
                                horizontal={false}
